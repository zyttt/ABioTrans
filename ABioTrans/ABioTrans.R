...
if(length(find.package(package = 'shinythemes',quiet = T))>0){
  ...
  server <- function(input,output,session){
    ...
    df_raw_shiny <- reactive({
      ...
      if(any(lengths==0)){
        showModal(modalDialog(
          title = "Error",
          "The gene lengths contain 0. Please check your data again!"
        ))
      }
      raw_DS <- raw_ds[,-c(1,ncol(raw_ds))]
    ...
    distaic <- reactive({
      ...
      for(i in 1:nrow(AIC.df)){
        AIC.df$min.AIC[i]<-colnames(AIC.df)[which.min(AIC.df[i,])]
      }
      return (AIC.df)
    })
